 ...
 *	Author: SmartThings
 *	Date: 2015-08-23
 *  Update: Michael Sutton
 *  Date: 2016-12-01
 *  Changes:  Correct power monitoring functions
 */
metadata {
	...
	definition (name: "Iris Smart Plug", namespace: "blebson", author: "SmartThings") {
		...
		fingerprint profileId: "0104", inClusters: "0000 0003 0004 0005 0006 0B04 0B05 FC03", outClusters: "0019", manufacturer: "CentraLite",  model: "3210-L", deviceJoinName: "SmartPower Outlet"
...
def initalize()
{
def finalResult = zigbee.getKnownDescription("energy");
log.info("initalize()")
}
...
def calculateAndShowEnergy()
{
    ...
    //log.info ("recentEvents ${recentEvents}")
   ...
   // log.info ("energyValue ${energyValue}")
    if(energyValue != null) {
    ...
    //log.info ("recentEvents[0].value ${recentEvents[0].value}")
...
def getPowerDescription(descMap) {
    ...
    log.debug ("getPowerDescription ${descMap}")
